...
export class RigidBody {
    ...
    constructor(shape: Shape, x: number, y: number, mass: number) {
        ...
        const localCenterOffset = shape instanceof PolygonShape ? shape.getLocalCenterOffset() : new Vec2(0, 0);
        this.position = new Vec2(x + localCenterOffset.x, y + localCenterOffset.y);
    ...
    addForce(force: Vec2, wake = true): void {
        if (wake) {
            this.wakeUp();
        }
    ...
    addTorque(torque: number, wake = true): void {
        if (wake) {
            this.wakeUp();
        }
    ...
    integrateVelocities(dt: number): void {
        if (this.isStatic() || this.isSleeping || this.hasCCD) {
    ...
    sleep(): void {
        if (this.isStatic() || !this.canSleep) {
            return;
        }
