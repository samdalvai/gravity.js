...
export function fix(aabb: AABB): void {
    const minX = Math.min(aabb.min.x, aabb.max.x);
    const maxX = Math.max(aabb.min.x, aabb.max.x);
    const minY = Math.min(aabb.min.y, aabb.max.y);
    const maxY = Math.max(aabb.min.y, aabb.max.y);
...
export function createAABB(body: RigidBody, margin: number = 0.0): AABB {
    if (body instanceof Circle) {
        ...
        const localToGlobal = body.localToGlobal;

        const res = new AABB(
            localToGlobal.mulVector2(body.vertices[0], 1),
            localToGlobal.mulVector2(body.vertices[0], 1),
        );
        fix(res);

        for (let i = 1; i < body.count; i++) {
            const gv = localToGlobal.mulVector2(body.vertices[i], 1);
...
export function union(b1: AABB, b2: AABB): AABB {
    const minX = Math.min(b1.min.x, b2.min.x);
    const minY = Math.min(b1.min.y, b2.min.y);
    const maxX = Math.max(b1.max.x, b2.max.x);
    const maxY = Math.max(b1.max.y, b2.max.y);

    const res = new AABB(new Vector2(minX, minY), new Vector2(maxX, maxY));
