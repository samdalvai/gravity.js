...
export class AngleJoint extends Joint {
    ...
    constructor(bodyA: RigidBody, bodyB: RigidBody, frequency = 60, dampingRatio = 1.0, jointMass = -1) {
    ...
    override prepare(inverseDeltaTime: number): void {
        ...
        const k = this.bodyA.inverseInertia + this.bodyB.inverseInertia + this.gamma;

        this.m = 1.0 / k;

        const error = this.bodyB.rotation - this.bodyA.rotation - this.initialAngleOffset;
    ...
    override solve(): void {
        ...
        const jv = this.bodyB.angularVelocity - this.bodyA.angularVelocity;
