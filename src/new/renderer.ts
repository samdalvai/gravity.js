...
export class Renderer {
    ...
    pick(screenPosition: Vector2): Vector2 {
        const inv_vp = this.vpc.inverted();
    ...
    drawCircleV(v: Vector2, radius: number = 0.05, filled: boolean = false): void {
        const vpcm = this.vpc.mulMatrix(this.modelTransform);

        const tv = vpcm.mulVector2(v, 1);
        const tr = this.vpc.mulVector2(new Vector2(radius, 0), 0).x;
    ...
    drawLineV(v0: Vector2, v1: Vector2, lineWidth: number = 1, strokeStyle: string = '#000000'): void {
        ...
        const tv0 = vpcm.mulVector2(v0, 1);
        const tv1 = vpcm.mulVector2(v1, 1);
    ...
    drawVector(p: Vector2, v: Vector2, arrowSize: number = 0.03): void {
        ...
        const n = new Vector2(-v.y, v.x).normalized().mulNew(3 * arrowSize);
    ...
    drawVectorP(p1: Vector2, p2: Vector2, arrowSize: number = 0.03): void {
        this.drawVector(p1, p2.subNew(p1), arrowSize);
    }
    ...
    drawBody(
        ...
        if (b instanceof Polygon) {
            ...
            for (let i = 0; i < b.count; i++) {
                const curr = b.vertices[i];
                const next = b.vertices[(i + 1) % b.count];

                const vpcm = this.vpc.mulMatrix(this.modelTransform);

                const tv0 = vpcm.mulVector2(curr, 1);
                const tv1 = vpcm.mulVector2(next, 1);

                if (i == 0) this.gfx.moveTo(tv0.x, Settings.height - tv0.y);
                this.gfx.lineTo(tv1.x, Settings.height - tv1.y);
            }
            this.gfx.closePath();
            ...
            const tv = vpcm.mulVector2(center, 1);
            const tr = this.vpc.mulVector2(new Vector2(b.radius, 0), 0).x;
    ...
    drawJoint(j: Joint) {
        if (j instanceof RevoluteJoint) {
            const anchorA = j.bodyA.localToGlobal.mulVector2(j.localAnchorA, 1);
            const anchorB = j.bodyB.localToGlobal.mulVector2(j.localAnchorB, 1);
            ...
            const anchor = j.bodyA.localToGlobal.mulVector2(j.localAnchor, 1);
            ...
            const anchor = j.bodyA.localToGlobal.mulVector2(j.localAnchorA, 1);
            ...
            if (j.drawAnchor) {
                this.drawCircleV(anchorA.addNew(j.linearOffset), 0.03);
                this.drawCircleV(anchorB, 0.03);
            }
        }
    }
