...
export function init(engine: Engine) {
    ...
    window.addEventListener('mousemove', e => {
        const rect = engine.cvs.getBoundingClientRect();
    ...
    for (let i = 0; i < maxMouseButtons; i++) mouses.push({ last_down: false, curr_down: false });
}
...
export function update() {
    ...
    for (let i = 0; i < mouses.length; i++) mouses[i].last_down = mouses[i].curr_down;
