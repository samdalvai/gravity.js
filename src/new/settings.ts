...
correction.addEventListener('click', () => {
    Settings.positionCorrection = correction.checked;
});
...
indicateContact.addEventListener('click', () => {
    Settings.indicateCP = indicateContact.checked;
});

const showBB = document.querySelector('#showBB')! as HTMLInputElement;
showBB.checked = Settings.showBoundingBox;
showBB.addEventListener('click', () => {
    Settings.showBoundingBox = showBB.checked;
});

const modeRadios = document.querySelectorAll('input[name="modeRadios"]');
for (let i = 0; i < 2; i++) {
    const me = modeRadios[i] as HTMLInputElement;
    me.addEventListener('change', () => {
        const index = Number(me.value);
        Settings.mode = index;
    });
}
...
const vertices_div = document.querySelector('#vertices_div')! as HTMLDivElement;

const shapeRadios = document.querySelectorAll('input[name="shapeRadios"]');
for (let i = 0; i < 4; i++) {
    const me = shapeRadios[i] as HTMLInputElement;
    me.addEventListener('change', () => {
        const index = Number(me.value);
...
restitution.value = String(
    Util.map(Settings.newBodySettings.restitution, restitutionRange.p1, restitutionRange.p2, 0, 100),
);
...
numVertices.value = String(
    Util.map(Settings.newBodySettings.numVertices, numVerticesRange.p1, numVerticesRange.p2, 0, 100),
);
...
numVertices.addEventListener('input', () => {
    ...
    if (mappedValue < 3) numVerticesLabel.innerHTML = 'Random';
    else numVerticesLabel.innerHTML = String(mappedValue);
...
contactLink.addEventListener('click', () => {
    Settings.showContactLink = !Settings.showContactLink;
});

const colorizeBody = document.querySelector('#colorizeBody')! as HTMLInputElement;
colorizeBody.checked = Settings.colorizeBody;
colorizeBody.addEventListener('click', () => {
    Settings.colorizeBody = !Settings.colorizeBody;
});
...
colorizeIsland.addEventListener('click', () => {
    Settings.colorizeIsland = !Settings.colorizeIsland;
});

const sleepEnabled = document.querySelector('#sleepEnabled')! as HTMLInputElement;
sleepEnabled.checked = Settings.sleepEnabled;
sleepEnabled.addEventListener('click', () => {
    Settings.sleepEnabled = !Settings.sleepEnabled;
});
...
applyWarmStartingThreshold.addEventListener('click', () => {
    Settings.applyWarmStartingThreshold = !Settings.applyWarmStartingThreshold;
});
...
colorizeActiveBody.addEventListener('click', () => {
    Settings.colorizeActiveBody = !Settings.colorizeActiveBody;
});
...
visualizeAABBTree.addEventListener('click', () => {
...
export function updateSetting(id: string, content?: any) {
    switch (id) {
        case 'pause':
            ...
            break;
        case 'g':
            if (content == undefined) Settings.applyGravity = !Settings.applyGravity;
            else Settings.applyGravity = content;
            applyGravity.checked = Settings.applyGravity;
            break;
        case 'c':
