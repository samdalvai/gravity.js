import { AngleJoint } from './constraint/angle';
import { Box } from './box';
...
function demo4(game: Game, world: World): void {
    ...
    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < rows - y; x++) {
            const b = Util.createRandomConvexBody(size, 4, 10.0);
            b.position = new Vector2(xStart + (y * (size + xGap)) / 2 + x * (size + xGap), yStart + y * (size + yGap));
            b.linearVelocity = b.position.mulNew(Util.random(0.5, 0.7));
...
function demo5(game: Game, world: World): void {
    ...
    const seesaw = new Box(6, 0.1);
    seesaw.position = new Vector2(0, 0.45);
    seesaw.mass = 10;
    world.register(seesaw);

    const j = new RevoluteJoint(ground, seesaw, seesaw.position, 60, 1.0);
    world.register(j);
...
function demo7(game: Game, world: World): void {
    ...
    for (let i = 0; i < 70; i++) {
        const r = Util.random(0.3, 2.8);
        const a = Util.random(0, Math.PI * 2);
        const p = new Vector2(Math.cos(a), Math.sin(a)).mulNew(r);
        const c = Util.createRandomConvexBody(0.09);
        c.position = center.addNew(p);
        c.mass = 1;
        world.register(c);
    }

    const b = new Box(0.1, 2);
...
function demo8(game: Game, world: World): void {
    ...
    const frictions = [0.51, 0.31, 0.21, 0.11, 0.0];
...
function demo9(game: Game, world: World): void {
    updateSetting('g', true);
    const g = new Box(Settings.clipWidth * 2, 0.4, Type.Static);
    g.restitution = 0.7;
    world.register(g);

    const count = 7;
    const gap = 1.0;
    const xStart = (-(count - 1) / 2) * gap;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const attenuation = (count - i) / count;
...
function demo10(game: Game, world: World): void {
    ...
    const j = new RevoluteJoint(ground, b, new Vector2(0, 5), -1);
    world.register(j);
}
...
function demo12(game: Game, world: World): void {
    ...
    const revoluteBridge = Util.random(0, 1) > 0.5;
    const frequecy = 7;

    if (revoluteBridge) {
        j = new RevoluteJoint(pillar, b1, pillar.position.addNew(new Vector2(pillarWidth, yStart).divNew(2)), frequecy, 1.0);
        j.drawAnchor = false;
        ...
        j = new DistanceJoint(
            pillar,
            b1,
            pillar.position.addNew(new Vector2(pillarWidth / 2, yStart / 2)),
            b1.position.addNew(new Vector2(-sizeX / 2, 0.03)),
            -1,
            frequecy,
            1.0,
        );
    ...
    for (let i = 1; i + 1 < (xStart * -2) / (sizeX + gap); i++) {
        ...
        if (revoluteBridge) {
            j = new RevoluteJoint(b1, b2, b1.position.addNew(b2.position).divNew(2), frequecy, 1.0);
            j.drawAnchor = false;
            world.register(j);
        } else {
            j = new DistanceJoint(
                ...
                b2,
                b1.position.addNew(new Vector2(sizeX / 2, 0.03)),
                b2.position.addNew(new Vector2(-sizeX / 2, 0.03)),
                -1,
                frequecy,
                1.0,
            );
...
function demo14(game: Game, world: World): void {
    ...
    const b1 = new Box(0.3, 6, Type.Static);
    ...
    let j = new DistanceJoint(b1, b2, b1.position.addNew(new Vector2(0, 2)), b2.position, 2, 1, 0.05);
    world.register(j);
    ...
    j = new DistanceJoint(b1, b2, b1.position.addNew(new Vector2(0, -2)), b2.position, 2, 1, 0.7);
    world.register(j);
    ...
    j = new DistanceJoint(b1, b2, b1.position.addNew(new Vector2(0, 2)), b2.position, 2, 0.5, 0.2);
    world.register(j);
    ...
    const halfLife = 1;
    const frequency = -Math.log(0.5) / (halfLife * Math.PI * 2);
    ...
    j = new DistanceJoint(b1, b2, b1.position.addNew(new Vector2(0, -2)), b2.position, 2, 2, 0.01);
    world.register(j);
}
...
function demo15(game: Game, world: World): void {
    ...
    for (let i = 0; i < 33; i++) {
        const start = new Vector2(Util.random(-8, 8), Util.random(0.5, 15));

        const rr = Util.random(-0.05, 0.05);
        const cos = Math.cos(rr);
        const sin = Math.sin(rr);

        const b2 = new Box(0.8, 0.03);
        b2.mass = 2.0;
        b2.restitution = 0.0;
        b2.position = start;
        b2.rotation = rr;
        ...
        const b1 = Util.createRegularPolygon(0.15);
        b1.mass = 2.0;
        b1.position = start.addNew(new Vector2(cos, sin).mulNew(-0.4));
        b1.restitution = 0.0;
        world.register(b1);

        const b3 = Util.createRegularPolygon(0.15);
        b3.mass = 2.0;
        b3.restitution = 0.0;
        b3.position = start.addNew(new Vector2(cos, sin).mulNew(0.4));
        world.register(b3);
...
function demo17(game: Game, world: World): void {
    ...
    const b0: RigidBody = new Box(0.3);
    b0.position.y = 0.1;
    world.register(b0);
    ...
    const b1 = new Box(2, 0.3, Type.Static);
    b1.position = new Vector2(-4, 6);
    world.register(b1);

    const b2 = new Box(0.8, 0.2);
    b2.restitution = 0.7;
    ...
    const j1 = new PrismaticJoint(b, j.bodyA, new Vector2(6, 2.5), undefined, new Vector2(-1, 0));
    world.register(j);
...
function demo18(game: Game, world: World): void {
    ...
    const b1 = new Box(0.2, 3.2, Type.Static);
    b1.position.y = 1.5;
    world.register(b1);

    const b2 = new Box(2.0, 0.2);
    b2.position.y = 3.0;
    world.register(b2);

    const j = new MotorJoint(b1, b2, b2.position, 1000, 10.0);
    world.register(j, true);
    ...
    game.callback = () => {
        ...
        if ((game.frame - last_spawn) / Engine.fps > 0.2 * 144 * game.deltaTime) {
            const c = Util.createRegularPolygon(0.15, undefined, undefined, 18.0);
...
function demo19(game: Game, world: World): void {
    ...
    const b1 = new Box(3, 0.2, Type.Static);
    b1.restitution = 0.5;
    b1.surfaceSpeed = 1.0;
    b1.position.x = -4.0;
    b1.position.y = 5.0;
    ...
    const b2 = new Box(5.5, 0.2, Type.Static);
    b2.restitution = 0.5;
    b2.surfaceSpeed = 1.0;
    b2.rotation = 0.2;
    b2.position.x = 0.5;
    ...
    const b3 = new Box(3.5, 0.2, Type.Static);
    b3.restitution = 0.5;
    b3.surfaceSpeed = -1.0;
    b3.position.x = 4.55;
    b3.position.y = 2.0;
    ...
    const b4 = new Box(2.0, 0.2, Type.Static);
    b4.restitution = 0.5;
    b4.surfaceSpeed = -1.0;
    b4.position.x = -4.55;
    b4.position.y = 2.0;
    ...
    let j: Joint = new RevoluteJoint(b4, b0, b4.position.addNew(new Vector2(1.0, 0.0)));
    j.drawConnectionLine = false;
    j.drawAnchor = false;
    world.register(j);

    for (let i = 1; i < 14; i++) {
        const b1 = new Box(sizeX, 0.1);
        b1.mass = 2.0;
        b1.surfaceSpeed = -1.5;
        ...
        j = new RevoluteJoint(b0, b1, b0.position.addNew(b1.position).divNew(2.0));
        j.drawAnchor = false;
        world.register(j);
        b0 = b1;
    }

    j = new RevoluteJoint(b3, b0, b0.position.addNew(new Vector2(0.2, 0.0)));
    j.drawConnectionLine = false;
    j.drawAnchor = false;
    world.register(j);
    ...
    game.callback = () => {
        if ((game.frame - last_spawn) / Engine.fps > 0.3 * 144 * game.deltaTime) {
            const c = Util.createRegularPolygon(Util.random(0.1, 0.25), undefined, undefined, 20.0);
            c.restitution = 0.3;
...
function demo20(game: Game, world: World): void {
...

    {
        ...
        const b1 = new Box(1.2, sizeY);
        b1.mass = 2.0;
        b1.position.x = xStart;
        b1.position.y = 3.3;
        world.register(b1);

        const b2 = new Box(0.2, 2.0);
        b2.mass = 2.0;
        ...
        j = new RevoluteJoint(c1, b2, c1.position.addNew(new Vector2(sizeX, 0)));
        j.drawConnectionLine = false;
        world.register(j, true);
        ...
        j = new PrismaticJoint(ground, b1, ground.position.addNew(new Vector2(xStart, 0)));
        j.drawAnchor = false;
        j.drawConnectionLine = false;
        world.register(j);
...

    {
        ...
        j = new RevoluteJoint(c2, b, c2.position.addNew(new Vector2(0.7, 0)));
        j.drawConnectionLine = false;
        world.register(j, true);

        const b1 = new Box(2.0, 0.2);
        b1.mass = 2.0;
        b1.position.x = xStart + 0.7 + 1.5 + 1.0;
        b1.position.y = yStart;
        world.register(b1);

        j = new RevoluteJoint(b, b1, b.position.addNew(new Vector2(1.5 / 2.0, 0)));
        j.drawConnectionLine = false;
        world.register(j, true);
...
function demo21(game: Game, world: World): void {
    ...
    const arm_start_x = 0.7;
    const arm_start_y = body_start_y + 0.85;
    const arm_gap = 0.2;

    const arm_width = 0.65;
    const arm_height = 0.25;

    const upper_arm_r = new Box(arm_width, arm_height);
    ...
    const lower_arm_r = new Box(arm_width, arm_height);
    ...
    const upper_arm_l = new Box(arm_width, arm_height);
    ...
    const lower_arm_l = new Box(arm_width, arm_height);
    ...
    const leg_start_x = 0.19;
    const leg_start_y = body_start_y - 0.8;

    const leg_width = 0.28;
    const leg_height = 0.75;
    const leg_gap = 0.3;

    const upper_leg_r = new Box(leg_width, leg_height);
    ...
    const lower_leg_r = new Box(leg_width, leg_height);
    ...
    const upper_leg_l = new Box(leg_width, leg_height);
    ...
    const lower_leg_l = new Box(leg_width, leg_height);
    ...
    const force = 10000;
    const torque_upper_arm = 1.25;
    const torque_lower_arm = 1.0;
    const torque_upper_leg = 1.75;
    const torque_lower_leg = 1.5;
    const frequency = 5;
    ...
    // Right arm
    {
        j = new MotorJoint(
            body3,
            upper_arm_r,
            new Vector2(arm_start_x - 0.2, arm_start_y),
            force,
            torque_upper_arm,
            frequency,
            1,
        ...
        const b1 = Util.createRegularPolygon(0.08, 5);
        b1.density = density;
    ...
    // Left arm
    {
        ...
        const b2 = Util.createRegularPolygon(0.08, 5);
        b2.density = density;
    ...
    // Right leg
    {
        j = new MotorJoint(
            body0,
            upper_leg_r,
            body0.position.addNew(new Vector2(0.2, 0)),
            force,
            torque_upper_leg,
            frequency,
            1,
        ...
        const b3 = Util.createRegularPolygon(0.125, 5);
        b3.density = density;
    ...
    // Left leg
    {
        ...
        const b4 = Util.createRegularPolygon(0.125, 5);
        b4.density = density;
    ...
    c.mass = 10;
    const angle = Util.random(0, Math.PI);
    c.position = new Vector2(Math.cos(angle), Math.sin(angle)).mulNew(5);
    c.linearVelocity = c.position.inverted().mulNew(Util.random(1.5, 3.5));
    c.position.y += 2.5;
    world.register(c);
}
...
function demo22(game: Game, world: World): void {
    ...
    const count = 5;
    const xGap = 2.3;
    const xStart = (-xGap / 2) * Math.trunc(count - 1 / 2);
    const yStart = 1.0;

    for (let i = 0; i < count; i++) {
        const b1 = new Polygon([new Vector2(0, 0), new Vector2(0, 0.6), new Vector2(0.5, 1.0)], undefined, false);
        b1.translate(new Vector2(xStart + xGap * i, yStart + i * 1.2));
        world.register(b1);
        const b2 = new Polygon([new Vector2(0, 0), new Vector2(-0.5, 1.0), new Vector2(0, 0.6)], undefined, false);
        b2.translate(new Vector2(xStart + xGap * i, yStart + i * 1.2));
        world.register(b2);

        const j = new WeldJoint(b1, b2);
        world.register(j, true);
...
function demo23(game: Game, world: World): void {
    ...
    //Car
    {
        ...
        const motor = new MotorJoint(body, wheel1, wheel1.position, 100, 100);
        motor.drawAnchor = false;
        world.register(motor);

        game.callback = () => {
            game.camera.scale = new Vector2(2, 2);
            const uv = { u: 1 - game.deltaTime * 3, v: game.deltaTime * 3 };
            game.camera.position = Util.lerpVector(game.camera.position, body.position.addNew(new Vector2(0, 2.0)), uv);
    ...
    // Ground
    {
        const g_1 = new Polygon(
            [new Vector2(-30, 5), new Vector2(-20, 5), new Vector2(-20, 5 - 0.4), new Vector2(-30, 5 - 0.4)],
            Type.Static,
            false,
        );
        ...
        const g0 = new Polygon(
            [
                new Vector2(-20, 5),
                new Vector2(-Settings.clipWidth / 2.0, 0.2),
                new Vector2(-Settings.clipWidth / 2.0, -0.2),
                new Vector2(-20, 5 - 0.4),
            ],
            Type.Static,
            false,
        );
        ...
        const g1 = new Box(Settings.clipWidth, 0.4, Type.Static);
        g1.restitution = 0.45;
        g1.friction = 1.0;
        world.register(g1);
        ...
        const g3 = new Polygon(
            [new Vector2(20.0, -1), new Vector2(50, -1), new Vector2(50, -1.4), new Vector2(20.0, -1.4)],
            Type.Static,
            false,
        );
        g3.restitution = 0.45;
        ...
        const h = Util.createRegularPolygon(0.25, 6);
        h.mass = 0.3;
        ...
        j = new RevoluteJoint(g3, b, b.position.subNew(new Vector2(sizeX / 2.0, 0)));
        j.drawConnectionLine = false;
        j.drawAnchor = false;
        world.register(j);
        ...
        for (let i = 1; i < count; i++) {
            const b2 = new Box(sizeX, sizeY, Type.Dynamic, 7.0);
            b2.position = new Vector2(xStart + (sizeX / 2.0 + gap) + (sizeX + gap) * i, yStart - sizeY / 2.0);
            world.register(b2);

            const j = new RevoluteJoint(b, b2, b2.position.subNew(new Vector2((sizeX + gap) / 2.0, 0)), 32, 1.0);
            j.drawAnchor = false;
            world.register(j);
            b = b2;
        }

        const g4 = new Box(50, 0.4, Type.Static);
        g4.position = new Vector2(xStart + (gap + sizeX) * count + g4.width / 2 + gap, yStart - sizeY / 2.0);
        g4.friction = 1.0;
        g4.restitution = 0.45;
        world.register(g4);

        j = new RevoluteJoint(g4, b, b.position.addNew(new Vector2(b.width / 2.0, 0)));
        j.drawConnectionLine = false;
        j.drawAnchor = false;
        world.register(j);
        ...
        for (let i = 0; i < 15; i++) {
            const b = Util.createRegularPolygon(size / 2, 6);
            b.position = new Vector2(xStart, yStart + i * (size + gap));
            world.register(b);
        }

        const g5 = new Box(0.4, 3, Type.Static);
        g5.position = g4.position
            .addNew(new Vector2(g4.width / 2.0, g4.height / 2.0))
            .addNew(new Vector2(g5.width / 2.0, g5.height / 2.0 - 0.4));
        world.register(g5);
    }
}
