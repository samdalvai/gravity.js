    ...
    getClosest(q: Vector2): ClosestResult {
        switch (this.count) {
            ...
            case 2: { // 1-Simplex: Line segment
                ...
                if (w.v <= 0) return { result: a, contributors: [0] };
                else if (w.v >= 1) return { result: b, contributors: [1] };
                else return { result: lerpVector(a, b, w), contributors: [0, 1] };
            }
            case 3: { // 2-Simplex: Triangle
                ...
                const area = b.subNew(a).cross(c.subNew(a));

                // If area == 0, 3 vertices are in collinear position, which means all aligned in a line

                const u = b.subNew(q).cross(c.subNew(q));
                const v = c.subNew(q).cross(a.subNew(q));
                const w = a.subNew(q).cross(b.subNew(q));
