...
class ContactSolver {
    ...
    prepare(dir: Vector2, contactType: ContactType, featureFlipped: boolean, inverseDeltaTime: number) {
        ...
        if (this.contactType == ContactType.Normal) {
            ...
            const relativeVelocity = this.bodyB.linearVelocity
    ...
    private applyImpulse(lambda: number) {
        ...
        this.bodyA.linearVelocity = this.bodyA.linearVelocity.addNew(
            this.jacobian.va.mulNew(this.bodyA.inverseMass * lambda),
        );
        ...
        this.bodyB.linearVelocity = this.bodyB.linearVelocity.addNew(
            this.jacobian.vb.mulNew(this.bodyB.inverseMass * lambda),
        );
...
class BlockSolver {
    ...
    solve() {
        ...
        const a = new Vector2(this.nc1.impulseSum, this.nc2.impulseSum); // old total impulse
